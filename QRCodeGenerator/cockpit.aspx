	...
	<script type="text/ecmascript">
		...
		function loadCockpit() {
			...
			for (var SI = 0; SI < myStruktur.SEArray.length; SI++) {
			    ...
			    // >= RGSErreicht ==> 100% gilt nur für den Werk und alle Pillar
				if (getQueryString('pid').toUpperCase() == 'ALLE' && myStruktur.SEArray[SI].sLayer == 1) {
				    myStruktur.SEArray[SI].sStatAlt = eval('(' + obj2str(myStruktur.SEArray[SI].sStat) + ')');

				    for (var PI = 0; PI < myPillar.PillarArray.length; PI++) {
				        for (var RI = 0; RI < myRGS.RGSArray.length; RI++) {
				            myStruktur.SEArray[SI].sStat['P' + myPillar.PillarArray[PI].id]['R' + myRGS.RGSArray[RI].id].obErreicht = false;
				            ...
				            for (var KI = 0; KI < myKriterien.KriterienArray.length; KI++) {
				                var theStat = myStruktur.SEArray[SI].sStat['P' + myPillar.PillarArray[PI].id]['R' + myRGS.RGSArray[RI].id]['K' + myKriterien.KriterienArray[KI].id];

				                var tmpABF = myStruktur.SEArray[SI].sStat['P' + myPillar.PillarArray[PI].id]['ABF'];

				                if (myKriterien.KriterienArray[KI].id < 6) {
				                    tmpSummeB += (tmpABF == 0 ? 0 : theStat.Summe / tmpABF);
				                    tmpAnzahlB += (tmpABF == 0 ? 0 : theStat.Anzahl / tmpABF);
				                }
				                else {
				                    tmpSummeE += (tmpABF == 0 ? 0 : theStat.Summe / tmpABF);
				                    tmpAnzahlE += (tmpABF == 0 ? 0 : theStat.Anzahl / tmpABF);
				                }
				            }

				            var tmpWieGutB = tmpAnzahlB == 0 ? -1 : tmpSummeB / tmpAnzahlB;
				            var tmpWieGutE = tmpAnzahlE == 0 ? -1 : tmpSummeE / tmpAnzahlE;

				            var tmpWieGutW = tmpWieGutE == -1 ? tmpWieGutB : (tmpWieGutB == -1 ? tmpWieGutB : (0.7 * tmpWieGutB + 0.3 * tmpWieGutE));

				            if ((tmpWieGutW / 3) >= RGSErreicht) {
				                myStruktur.SEArray[SI].sStat['P' + myPillar.PillarArray[PI].id]['R' + myRGS.RGSArray[RI].id].obErreicht = true;

				                for (var KI = 0; KI < myKriterien.KriterienArray.length; KI++) {
				                    var theStat = myStruktur.SEArray[SI].sStat['P' + myPillar.PillarArray[PI].id]['R' + myRGS.RGSArray[RI].id]['K' + myKriterien.KriterienArray[KI].id];

				                    theStat.Summe = theStat.Anzahl * 3;
				                }
				            }
				        }
				    }
